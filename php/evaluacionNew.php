...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id_modulo'])) {
        ...
        $deskpoints = intval($_POST['deskpoints']);
        ...
        try {
            ...
            $stmtEvaluacion->bind_param("issssssssi", $id_modulo, $titulo, $descripcion, $fecha_emision, $hora_emision, $fecha_inicio, $hora_inicio, $fecha_entrega, $hora_entrega, $deskpoints);
            
            if (!$stmtEvaluacion->execute()) {
                throw new Exception('Error al crear la evaluación: ' . $stmtEvaluacion->error);
            }
            ...
            if (isset($_FILES['archivo']) && $_FILES['archivo']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!in_array($archivo['type'], $allowedTypes)) {
                    throw new Exception('Tipo de archivo no permitido. Use PDF, PPT, DOC, PNG, JPG, ZIP o RAR.');
                }
                ...
                if (move_uploaded_file($archivo['tmp_name'], $rutaDestino)) {
                    ...
                    $rutaRelativa = 'uploads/evaluaciones/' . $nombreArchivo;
                    ...
                    // Relacionar archivo con la evaluación en archivos_publicacion (USANDO PREFIJO)
                    $sqlRelacion = "INSERT INTO archivos_publicacion (id_archivo, id_publicacion) VALUES (?, ?)";
                    $stmtRelacion = $conexion->prepare($sqlRelacion);
                    $id_publicacion_con_prefijo = 'EV-' . $id_evaluacion;
            ...
            echo json_encode([
                'success' => true, 
                'message' => 'Evaluación creada exitosamente' . ($archivo_subido ? ' con archivo adjunto' : ''),
                'id_evaluacion' => $id_evaluacion,
                'id_publicacion' => 'EV-' . $id_evaluacion,
        ...
        // Cerrar statements
        if (isset($stmtEvaluacion)) $stmtEvaluacion->close();
        ...
        if (isset($data['id_modulo']) && isset($data['titulo']) && 
            ...
            // Insertar evaluación sin archivo
